...
public class ProfesorDAO {
    ...
    public static void insertar(Session session, Profesor f){
        session.beginTransaction();
        session.save(f);
        session.getTransaction().commit();
    }

    public static void modificar(Session session, Profesor f){
        session.beginTransaction();
        session.update(f);
        session.getTransaction().commit();
    }

    public static void eliminar(Session session, Profesor f){
        session.beginTransaction();
        session.delete(f);
        session.getTransaction().commit();
    }
